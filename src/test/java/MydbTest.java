...
public class MydbTest  extends JerseyTest{
    ...
    protected Application configure() {
        return new ResourceConfig(MyDb.class);
    }
	...
	private ResultSet rsObj;
	 ...
	 public void login()
	 {
		 String correctpwd="12345"; //Initial wrong password
		 String response="No user exists";
		 String sql ="select *  from USERS WHERE users_email = ssmith@gmail.com AND users_password =1234";
        try {
		 st =con.prepareStatement(sql);
			rsObj = st.executeQuery(sql);
			
	        if(rsObj.next()){
	        correctpwd=rsObj.getString("users_password");
				}
	        
	        if(correctpwd.equalsIgnoreCase("1234")) {
	        	response="Logged in";
	 ...
	 public void WrongLogin() {
		 String correctpwd="12345"; //Initial wrong password
		 String response="No user exists";
		 String sql ="select *  from USERS WHERE users_email = fds.com AND users_password =1234";
        try {
		 st =con.prepareStatement(sql);
		rsObj = st.executeQuery(sql);
			correctpwd=rsObj.getString("users_password");
	 ...
	 @Test
	 public void Search() {
		 String searchString="Hilton"; //Here entered hotel name
		 String response="";
        ...
        try {
    		...
    		rsObj = st.executeQuery(sql);
    		while(rsObj.next()) {
    		response=rsObj.getString("hotel_name");}
			}catch(SQLException sqlExObj) {
				//Since no user exists it should go into this catch 
				 assertEquals(searchString,response);
        }
